...
PAGES_REPO = Path(__file__).resolve().parent
OPPORTUNITY_REPO = PAGES_REPO.parent / "Opportunity"
...
TEMP_DOC_DIR = PAGES_REPO / "temp_doc"
...
def sync_enriched_to_publish():
    ...
    for src in ENRICHED_DOCS_DIR.iterdir():
        if src.is_file() and src.suffix == ".html":
            ...
            archived = TEMP_DOC_DIR / src.name
            if dest.exists() or archived.exists():
                continue
