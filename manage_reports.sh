...
# Check if a full Monday report (_enriched) exists for the week containing a given date
full_monday_report_exists() {
  local mm=$1 dd=$2 yy=$3
  local prev_mon
  prev_mon=$(get_prev_monday "$mm" "$dd" "$yy")
  local found
  found=$(find "$DOCS_DIR" -maxdepth 1 -name "opportunities_*_${prev_mon}_enriched.html" -type f 2>/dev/null | head -1)
  [ -n "$found" ]
}

# Check if only a partial report exists for the week (no full enriched)
only_partial_exists_for_week() {
  local mm=$1 dd=$2 yy=$3
  local prev_mon
  prev_mon=$(get_prev_monday "$mm" "$dd" "$yy")
  local full
  full=$(find "$DOCS_DIR" -maxdepth 1 -name "opportunities_*_${prev_mon}_enriched.html" -type f 2>/dev/null | head -1)
  [ -n "$full" ] && return 1
  local partial
  partial=$(find "$DOCS_DIR" -maxdepth 1 -name "opportunities_*_${prev_mon}_partial.html" -type f 2>/dev/null | head -1)
  [ -n "$partial" ]
}

# Check if any Monday report (full or partial) exists for the week
monday_report_exists() {
  local mm=$1 dd=$2 yy=$3
  full_monday_report_exists "$mm" "$dd" "$yy" || only_partial_exists_for_week "$mm" "$dd" "$yy"
}
...
main() {
  ...
  while IFS= read -r fname; do
    ...
    if [ "$dow" -eq 1 ]; then
      ...
      if only_partial_exists_for_week "$mm" "$dd" "$yy"; then
        local prev_mon_date
        prev_mon_date=$(get_prev_monday "$mm" "$dd" "$yy")
        local old_partial
        old_partial=$(find "$DOCS_DIR" -maxdepth 1 -name "opportunities_*_${prev_mon_date}_partial.html" -type f 2>/dev/null | head -1)
        if [ -n "$old_partial" ]; then
          local old_partial_name
          old_partial_name=$(basename "$old_partial")
          log "  Only a partial exists for this week ($old_partial_name) — this is the real weekly report."
          if [ "$DRY_RUN" = true ]; then
            log "  (dry-run) Would archive partial and keep this report in docs/"
          else
            ensure_temp_dir
            mv "$old_partial" "$TEMP_DIR/$old_partial_name"
            git reset HEAD -- "docs/$old_partial_name" 2>/dev/null || true
            log "  Archived partial: $old_partial_name"
          fi
          clear_state
          current_virtual=""
          current_week=""
        ...
        log "  Keeping late weekly report in docs/"
        continue
      fi
